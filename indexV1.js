...
app.get('/api/urk/top', async (req, res) => {
     ...
     async function fn() {
       ...
       request.on('row', function(columns) {
         fs.writeFile('helloworld.txt', JSON.stringify(columns.length), function (err) {
...
app.get('/api/urk/name', async (req, res) => {
     ...
     function executeStatement() {
       ...
       request = new Request("SELECT Id,Locatie,Plank,Categorie,Omschrijving,GPK,Inhoud,Producent,Eigenaar,\"Foto Nr\" FROM [dbo].[Beeldbank Urk] WHERE Omschrijving LIKE %@qr% ;",function(err, rowCount, data) {
